---
tipo: tabela-db
area: orius
produto: protesto
dominio: cadastro
tabela: P_CUSTAS_CANCELAMENTO
banco: palmelo2
tags: [orius, db, firebird, protesto, cadastro]
status: gerado
tem_legado_delphi: true
---

> **Produto:** [[Orius/empresa/produtos/tabelionato-protesto]] | **Dominio:** `cadastro` | **Indice DB:** [[Orius/desenvolvimento/banco-de-dados/produtos/protesto/00-indice-protesto-db]]

# `P_CUSTAS_CANCELAMENTO`

## Objetivo

_(A preencher.)_

## Descricao

_(A preencher.)_

## Schema

| Coluna | Tipo (Firebird) | Null | Default | PK | FK | Descricao |
|--------|-----------------|------|---------|----|----|-----------|
| `CUSTAS_CANCELAMENTO_ID` | BIGINT | N | - | sim |  | |
| `FAIXA` | VARCHAR(10) | N | - |  |  | |
| `CUSTAS` | BIGINT | N | - |  |  | |
| `TAXAS` | BIGINT | N | - |  |  | |
| `VALOR_FIXO` | BIGINT | N | - |  |  | |
| `ISS` | BIGINT | N | - |  |  | |
| `VALOR_INICIAL` | BIGINT | N | - |  |  | |
| `VALOR_FINAL` | BIGINT | N | - |  |  | |
| `EMOLUMENTO_PERIODO_ID` | BIGINT | N | - |  | `G_EMOLUMENTO_PERIODO(EMOLUMENTO_PERIODO_ID)` | |

## Chaves e indices

| Nome | Tipo | Colunas |
|------|------|---------|
| `P_CUSTAS_CANCELAMENTO_PK` | PRIMARY KEY | CUSTAS_CANCELAMENTO_ID |
| `P_CUSTAS_CANCELAMENTO_EMOLU_FK` | FOREIGN KEY | EMOLUMENTO_PERIODO_ID -> `G_EMOLUMENTO_PERIODO(EMOLUMENTO_PERIODO_ID)` |
| _(nenhuma)_ | - | - |

## Relacionamentos

_(Documentar cardinalidades na fase seguinte - ver FK acima.)_

## Regras de negocio

- 

## Legado (Delphi)

| Item | Referencia |
|------|------------|
| Unit / datamodule | |
| Telas | |

## Web (novo)

| Item | Referencia |
|------|------------|
| Entidade / API | |
| Migracao | [[Orius/desenvolvimento/refatoracao/migracao-delphi-para-web]] |

## Integracoes

_(Se aplicavel.)_

## Metadados

| Campo | Valor |
|-------|-------|
| Export JSON | [[Orius/desenvolvimento/banco-de-dados/metadata/palmelo2/protesto-p-metadata.json]] |
| Script | [[Orius/desenvolvimento/scripts/firebird/export-protesto-p.md]] |
| Extraido em | 2026-05-29 |
| Charset conexao | ISO8859_1 |
| Status | `gerado` - schema automatico; objetivo/descricao pendentes |
